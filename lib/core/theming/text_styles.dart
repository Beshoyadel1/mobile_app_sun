...
class TextInAppWidget extends StatefulWidget {
  const TextInAppWidget({super.key, required this.text, this.textColor, this.textSize, this.textWidth, this.isTextCenter, this.textAlign, this.fontWeightIndex, this.isEllipsisTextOverflow, this.maxLines, this.decorationTextColor, this.decorationText = TextDecoration.none}) ;
  ...
  Widget build(BuildContext context) {
    ...
    return SizedBox(
      ...
      child: Text( AppLocalizations.of(context).translate(widget.text)  ,
          textAlign: widget.textAlign ?? (widget.isTextCenter == true ? TextAlign.center : TextAlign.start) ,
          maxLines: widget.maxLines,
          softWrap: true,
          overflow: TextOverflow.visible,
          style: TextStyle(
            overflow: widget.isEllipsisTextOverflow == true ? TextOverflow.ellipsis : null,
            fontSize: widget.textSize != null ? ( _languageCubit.isAllAppLanguageArabic ? widget.textSize : widget.textSize! - 2) :  (_languageCubit.isAllAppLanguageArabic ? size.width*0.05 :  size.width*0.04) ,
